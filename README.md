...
Версія схеми артефактів: **1.3.0** (звіряється з `tools/SCHEMAS.md` курсу).
...
Папки `lr04_kanban/`, `lr10_demo/` і `lr13_roleplay/` команда не здає: це
воркшопи, результат яких живе всередині пари. `lr03_sprint_simulation/` здається
всіма: ЛР3 це самостійна робота, і кожен учасник кладе туди свій журнал спринта
під іменем `sprint_log_<логін>.md`. Балів журнал не дає, але без нього нема з
чим приходити на ЛР4. У студента індивідуального плану `lr03_sprint_simulation/`
і `lr04_kanban/` теж здаються: туди йде модуль M5.
